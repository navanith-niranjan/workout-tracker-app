...
urlpatterns = [
    ...
    path('users/<int:pk>/workout-history/<int:session_number>/details/', SessionsViewSet.as_view({'get': 'list_entries', 'post': 'create_entry'}), name = 'session-details'),
    path('users/<int:pk>/workout-history/<int:session_number>/details/<int:exercise_number>/', SessionsViewSet.as_view({'get': 'retrieve_entry', 'put': 'update_entry', 'delete': 'destroy_entry'}), name = 'entry-details')
]